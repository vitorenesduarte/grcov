...
// path's start.
fn guess_abs_path(source_dir: &PathBuf, rel_path: &PathBuf) -> PathBuf {
    let join = source_dir.join(rel_path);
    if join.is_file() {
        return join;
    }
    for ancestor in rel_path.ancestors() {
        if source_dir.ends_with(ancestor) && !ancestor.as_os_str().is_empty() {
            return source_dir.join(rel_path.strip_prefix(ancestor).unwrap().to_path_buf());
        }
    }
    join
}
